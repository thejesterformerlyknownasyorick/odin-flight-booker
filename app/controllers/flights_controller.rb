...
class FlightsController < ApplicationController
    ...
    def index
        ...
        @flight_time_options = Flight.select(:departure_time).group('DATE(departure_time)')
                                     .map{ |f| [ f.searchable_date, f.departure_time.to_date ]}
        ...
        if !params[:departure_id].nil?
            ...
            @flight_search_results = @flight_search_results.where("DATE(departure_time) = ?", params[:departure_time].to_date)
        end
    end
